...
export default async function handler(req: NextRequest) {
  const { prompt, history = [], options = {}, apiKey ,gptVersion} = await req.json();
  ...
  const stream = await requestStream(data, apiKey);
  return new Response(stream);
}

const requestStream = async (payload: StreamPayload, apiKey: string) => {
  let counter = 0;
